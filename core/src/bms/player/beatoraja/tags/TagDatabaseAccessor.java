...
public class TagDatabaseAccessor extends SQLiteDatabaseAccessor {
    ...
    public void tagChartBatch(int tag, String[] sha256, String[] md5, TagMetadata[] meta) {
        ...
        try {
            Object[][] varbatch = new Object[length][7];
            for(int i = 0; i < length; ++i) {
                varbatch[i][0] = tag;
                varbatch[i][1] = sha256[i];
                varbatch[i][2] = md5[i];
                varbatch[i][3] = meta[i].getTitle();
                varbatch[i][4] = meta[i].getNotes();
                varbatch[i][5] = meta[i].getUrl();
                varbatch[i][6] = meta[i].getAppendurl();
            }
            qr.batch(singleConnection,
                     "INSERT OR REPLACE INTO unifiedmap (tag, sha256, md5, title, notes, url, appendurl) "
                     + "VALUES (?, ?, ?, ?, ?, ?, ?)",
                     varbatch);

            Object[][] shabatch = new Object[length][3];
            Object[][] md5batch = new Object[length][3];
            Object[][] metabatch = new Object[length][5];
            for(int i = 0; i < length; ++i) {
                int entry = nextMetaId();
                metabatch[i][0] = entry;
                metabatch[i][1] = meta[i].getTitle();
                metabatch[i][2] = meta[i].getNotes();
                metabatch[i][3] = meta[i].getUrl();
                metabatch[i][4] = meta[i].getAppendurl();
            ...
            qr.batch(singleConnection,
                     ...
                     md5batch);
            qr.batch(singleConnection,
                     "INSERT OR REPLACE INTO metadata (id, title, notes, url, appendurl) "
                     + "VALUES (?, ?, ?, ?, ?)",
                     metabatch);
        }
        catch (SQLException e) {
    ...
    public void setChartTagMeta(int tag, String sha256, String md5, String key, String value) {
        ...
        try {
            ...
            if (meta != null) {
                qr.update(singleConnection, "UPDATE metadata SET ? = ? WHERE id = ?", key, value, meta);
            }
    ...
    public TagMetadata getChartTagMeta(int tag, String sha256, String md5) {
        ...
        try {
            ...
            if (meta != null) {
                // result = qr.query(singleConnection, "SELECT title, notes, url, appendurl FROM metadata WHERE id = ?",
                result = qr.query(singleConnection, "SELECT *  FROM metadata WHERE id = ?",
